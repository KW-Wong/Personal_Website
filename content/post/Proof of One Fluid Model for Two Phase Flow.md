+++
title = "Proof of One-Equation Model of Liquid Vapour Two Phase Flow"
...
caption = "1950s"
+++
In the single component two phase fluid, there would be a separate set of mass, momentum and energy equation for each of the phase. This is the basics of constructing the two fluid model, in which the two sets of equation are solved indepdently at each timestep and recombine with the usage of interface boundary condition. Nevertheless, the drawbacks of the two fluid model is that it based on mainly RANS terminology in which the interface contributions to momentum and energy are lost due to the lost of information at the interface.

In view of this, the interface tracking method is developed mainly for an accurate determination of the sharp interface between two phases. The implementation of the interface tracking requires a one fluid formulation of the mass, momentum and energy equation. Depends on the interface tracking algorithm, the interface can either be tracked by solving a scalar advection equation or even reconstruct the interface using the volume fraction at every time-step. There is also way to track the interface by moving grid which is the scope of Lagranian tracking and it is out of the scope of this note.
